...
"""
GET /stats — Stats summary (Blackjack + Spanish 21) for the logged-in user
...
Response (example):
{
  ...
  "per_mode": {
    ...
    "Spanish_4to9":    { "total":   0, "errors":  0, "correct":   0, "accuracy": null },
    "Spanish_2to3":    { "total":   0, "errors":  0, "correct":   0, "accuracy": null },
    "Spanish_perfect": { "total":   0, "errors":  0, "correct":   0, "accuracy": null }
  },
...
def get_stats():
    ...
    cur.execute("""
        SELECT
          ...
          total_spa4to9_hands,  errors_spa4to9_hands,
          total_spa2to3_hands,  errors_spa2to3_hands,
          total_spa_perfect_hands, errors_spa_perfect_hands
        FROM user_stats
        WHERE user_id = %s
        LIMIT 1;
    """, (user_id,))
    ...
    if not row:
        ...
        return jsonify({
            ...
            "per_mode": {
                ...
                "Spanish_4to9": zero, "Spanish_2to3": zero, "Spanish_perfect": zero
            },
            "totals": zero
        }), 200
    ...
    m_spa49  = _pack(row["total_spa4to9_hands"],   row["errors_spa4to9_hands"])
    m_spa23  = _pack(row["total_spa2to3_hands"],   row["errors_spa2to3_hands"])
    m_spap   = _pack(row["total_spa_perfect_hands"], row["errors_spa_perfect_hands"])

    total_all  = (m_410["total"] + m_23["total"] + m_perf["total"] + m_a9das["total"] +
                  m_a9nd["total"] + m_spa49["total"] + m_spa23["total"] + m_spap["total"])
    errors_all = (m_410["errors"] + m_23["errors"] + m_perf["errors"] + m_a9das["errors"] +
                  m_a9nd["errors"] + m_spa49["errors"] + m_spa23["errors"] + m_spap["errors"])
    ...
    return jsonify({
        ...
        "per_mode": {
            ...
            "A-9NoDAS": m_a9nd,
            "perfect": m_perf,
            "Spanish_4to9": m_spa49,
            "Spanish_2to3": m_spa23,
            "Spanish_perfect": m_spap
        },
        "totals": totals
    }), 200
